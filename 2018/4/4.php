...
$guardId = whoSleptMost($shiftData);

$minute = getSleepiestMinute($shiftData, $guardId);

echo $minute * $guardId;

//printData($shiftData);

function getSleepiestMinute($shiftData, $guardId)
{
	$minutes = array();
	for ($i = 0; $i < 60; $i++)
	{
...
function whoSleptMost($shiftData)
{
	$guards = array();
	
	foreach ($shiftData as $shift)
	{
		if (!isset($guards[$shift->id]))
		{
			$guards[$shift->id] = 0;
		}
		
		$guards[$shift->id] += $shift->sleeping;
	}
	...
	{
		if ($minutes > $sleepiestAmount)
		{
			$sleepiest = $id;
			$sleepiestAmount = $minutes;
		}
	}
